...
function scan_page(i)
	...
	if state.params.type == 'list' and state.params.fast_extract then
		...
		if link then
			...
			local name, texture, count, _, _, _, _, _, _, buyout_price, _, _, _, owner, _, sale_status = GetAuctionItemInfo('list', i)
			auction_info = O(
...
function check_for_duplicate_page(q, pagenum)
	...
	for i = 1, numOnPage do
		local name, _, count, _, _, _, _, minBid, minInc, buyoutPrice, bidAmount, _, _, owner = GetAuctionItemInfo('list', i)
