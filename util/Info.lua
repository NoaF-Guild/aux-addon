...
function M.auction(index, query_type)
	...
	if link then
        ...
        return O(
            ...
            'itemstring', item_info.itemstring,
            'item_key', item_id .. ':' .. suffix_id,
			'search_signature', join(temp-A(item_id, suffix_id, enchant_id, start_price, buyout_price, bid_price, aux_quantity, duration, sale_status, query_type == 'owner' and (high_bidder or 0) or (high_bidder and 1 or 0), aux_ignore_owner and (cache.is_player(owner) and 0 or 1) or (owner or '?')), ':'),
